...
description: >-
  Find and fix broken links on a website or in a codebase. Use when the user asks
  to "check broken links", "find dead links", "fix 404s", "link checker",
  "broken link audit", "find dead URLs", or wants to identify and repair links
  that lead to non-existent pages.
---
...
**Data dependency:** finding these requires an external backlink index (Ahrefs, Majestic, or Search Console's links report). A crawl of your own site cannot discover them — by definition the links live elsewhere. If no backlink source is available, say so rather than reporting zero: "no backlink data supplied, so backlink 404s were not assessed" is accurate; "no backlink 404s found" is not.

Search Console's links report is the free option and is usually enough to catch the high-value cases.
...
| Concern | Owner |
|---|---|
| Where links *should* be added, anchor text, orphan pages | `internal-linking` |
| Redirect mapping for a restructure or migration | `site-architecture` |
| Robots/canonical directives behind an "unreachable" page | `technical-seo` |

**One caveat on client-rendered sites:** if links are injected by JavaScript, a raw-HTML crawl will report them as missing and produce false positives across the board. Confirm with `rendering-audit` before filing a large 404 list on a React/Vue/Next site.
...
- "check broken links" / "find dead links" / "fix 404s"
- "link checker" / "broken link audit" / "find dead URLs"
